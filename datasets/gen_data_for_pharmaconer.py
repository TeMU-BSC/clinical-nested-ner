...
def gen_nested_ner_format(tokenizer, texts, annotations, entity_types, output_filename):
    ...
    #real_output_file = open('real.conll', 'w')
...
def create_mlc_data(path, partition, entity_types):
    ...
    if not os.path.exists(f'formatted_data/MLC/pharmaconer'):
          os.mkdir(f'formatted_data/MLC/pharmaconer')

    for entity in entity_types:
        if not os.path.exists(f'formatted_data/MLC/pharmaconer/{entity}'):
          os.mkdir(f'formatted_data/MLC/pharmaconer/{entity}')

        f_out = open(f'formatted_data/MLC/pharmaconer/{entity}/{entity}_{partition}.iob2', 'w')
...
if __name__=='__main__':
    ...
    train_path = os.path.join(actual_path, "raw_data/pharmaconer/train")
    dev_path = os.path.join(actual_path, "raw_data/pharmaconer/dev")
    test_path = os.path.join(actual_path, "raw_data/pharmaconer/test")
    ...
    train_entities = gen_nested_ner_format(tokenizer, train_texts, train_annotations, entity_types, 'formatted_data/pharmaconer/train.iob2')
    print("Total entities in train partition: {}".format(len(train_entities)))

    dev_entities = gen_nested_ner_format(tokenizer, dev_texts, dev_annotations, entity_types, 'formatted_data/pharmaconer/dev.iob2')
    print("Total entities in dev partition: {}".format(len(dev_entities)))

    test_entities = gen_nested_ner_format(tokenizer, test_texts, test_annotations, entity_types, 'formatted_data/pharmaconer/test.iob2')
    print("Total entities in test partition: {}".format(len(test_entities)))
    
    create_mlc_data('formatted_data/pharmaconer/train.iob2', 'train', entity_types)
    create_mlc_data('formatted_data/pharmaconer/dev.iob2', 'dev', entity_types)
    create_mlc_data('formatted_data/pharmaconer/test.iob2', 'test', entity_types)