...
if __name__=='__main__':
    ...
    wl_entity_types = ["Disease", "Medication", "Body_Part", "Abbreviation", "Family_Member", "Finding", "Procedure"]
    ...
    for dataset in ['wl']:
        ...
        for model in tqdm(os.listdir(f'preds/{dataset}/'), desc="Generating nested metrics....................."):
            for seed in [1, 2, 3]:
                for lr in ['1e-5', '1e-6', '5e-5', '5e-6']:
                    path = f'preds/{dataset}/{model}/{dataset}/seed-{seed}/lr-{lr}/preds_with_labels.conll'
                    ...
                    for sent in sentences:
                        ...
                        for line in sent.splitlines():
                            ...
                            for type in entity_types:
                                if f'B-{type}' in line.split()[1].split('+'):
                                    gold_entities_dict[type].append(f'B-{type}')
                                elif f'I-{type}' in line.split()[1].split('+'):
                                    gold_entities_dict[type].append(f'I-{type}')
                                else:
                                    gold_entities_dict[type].append('O')
                            
                                if f'B-{type}' in line.split()[2].split('+'):
                                    pred_entities_dict[type].append(f'B-{type}')
                                elif f'I-{type}' in line.split()[2].split('+'):
                                    pred_entities_dict[type].append(f'I-{type}')
                                else:
                        ...
                        data.append({"real": real_entities, "pred": pred_entities})
                
                    output_file.write(f'Dataset: {dataset}, Model: {model}, Seed: {seed}, Learning_rate: {lr} \n')
                    p, r, f, support = standard_metric(data)
                    output_file.write(f'Standard metric - Precision: {p}, Recall: {r}, F1 score: {f}, Support: {support}\n')
                
                    p, r, f, support = flat_metric(data)
                    output_file.write(f'Flat metric - Precision: {p}, Recall: {r}, F1 score: {f}, Support: {support}\n')
                
                    p, r, f, support = inner_metric(data)
                    output_file.write(f'Inner metric - Precision: {p}, Recall: {r}, F1 score: {f}, Support: {support}\n')

                    p, r, f, support = outer_metric(data)
                    output_file.write(f'Outer metric - Precision: {p}, Recall: {r}, F1 score: {f}, Support: {support}\n')

                    p, r, f, support = nested_metric(data)
                    output_file.write(f'Nested metric - Precision: {p}, Recall: {r}, F1 score: {f}, Support: {support}\n')

                    p, r, f, support = nesting_metric(data)
                    output_file.write(f'Nesting metric - Precision: {p}, Recall: {r}, F1 score: {f}, Support: {support}\n')
